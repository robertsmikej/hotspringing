...
export const mutations = {
    ...
    setSprings(state, data) {
        ...
        state.numOfSprings.number = data.data.length;
    },
...
//        state.articles = data;
    ...
    setRecentArticles(state, data) {
        for (var articleRecent in data) {
            state.recentArticles[data[articleRecent].condensed_title] = data[articleRecent];
        }
//        state.articles = data;
    },
...
export const getters = {
    ...
    numOfSprings: state => state.numOfSprings
}
...
export const actions = {
    async nuxtServerInit ({commit}) {
        ...
        const recentArticles = await axios.get("https://www.hotspringing.com/directus/public/_/items/articles?fields=*.*.*&limit=6").then((response) => {
            commit("setRecentArticles", response.data.data);
        });
        const artTips = await axios.get("https://www.hotspringing.com/directus/public/_/items/articles?filter[article_type]=tips_and_tricks&fields=*.*.*").then((response) => {
            commit("setArticlesTips", response.data.data);
        });
        const artTripReports = await axios.get("https://www.hotspringing.com/directus/public/_/items/articles?filter[article_type]=trip_report&fields=*.*.*").then((response) => {
